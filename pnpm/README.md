# pnpm — direct access to Chainguard Libraries

[`demo.sh`](demo.sh) shows how to point pnpm at
...
Running `demo.sh` creates a fresh, throwaway pnpm project in a git-ignored
...
exactly what you would apply to a real pnpm project, so treat the script as a
...
| Auth mechanism | `username` / `_password` |

The script writes these with `pnpm config set ... --location=project`, so the
...
To test a different package, edit the `pnpm add` line near the end of
